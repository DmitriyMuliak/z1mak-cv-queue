...
2. scanKeys() без збереження курсору
...
І/або використовувати TTL на job:*:result, щоб Redis сам очищав старі ключі
3. syncDbResults() має слабкий захист від часткових збоїв
...
Винести парсинг meta/result у ізольовану функцію з перевірками
4. Масова обробка ключів без батч-паралелізму
...
🧪 Покриття тестами
Функції мають експортовану обгортку __test, що дозволяє покривати unit-тестами основну логіку.
