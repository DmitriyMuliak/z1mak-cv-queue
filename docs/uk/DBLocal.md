### // Apply SQL in the current Volume
...
1 - Підняти локально Supabase
2 - Додати зміни через UI ( Studio │ http://127.0.0.1:54323 ).
3 - Створити автоматичну міграцію з нових схем у бд. Накатити Міграції.
4 - Створити міграцію та додати у неї SQL.
5 - Залінкувати remote DB. (Project ID - evyllttuifftofzucyed)
6 - Зробити снепшот / накатити міграції на remote Supabase
7 - Зробити зміни на remote Supabase та створити міграцію і накатити на локальний Supabase.
8 - Накатити міграцію на Локальний postgres (in docker-compose-develop.yaml)
...
Якщо попросить пароль — це DB password, не service key. Можеш передати так:
npm run supabase -- db push --linked --password "<DB_PASSWORD>"
Явний рядок підключення:
npm run supabase -- db push --db-url "postgresql://... (URL-encoded)"
