...
describe('Database Synchronization Cron Job', () => {
  ...
  it('automatically persists completed job result to PostgreSQL via cron', async () => {
    ...
    const body = { ...createBody('lite'), userId: 'db-sync-user', role: 'user' as const };
    ...
    // 2. Verify DB Persistence (wait for DB sync cron - now 2s interval in test)
    console.log('[Test] Waiting for DB sync cron (up to 10s)...');
    let dbRow = null;
    const startTime = Date.now();
    while (Date.now() - startTime < 10_000) {
      const dbRes = await pgClient.query('SELECT * FROM cv_analyzes WHERE id = $1', [jobId]);
